...
5. `client`：首頁（含麥克風權限與音量測試）、`LocalGameDriver`、對戰畫面（血條、倒數環、題目卡、即時字幕、hot-seat 換手過場）、結果頁
6. `BattleFxLayer` 佔位版：傷害數字跳出、血條震動、Perfect 字樣

**驗收**：兩個人用同一台筆電可以完整打完一場（嗆聲 → 擲先攻 → 多回合 → 分勝負 → 再來一局），中英文題都能正確計分。
...
1. `shared/scoring/rhythm.ts`：拍點判定（01 §3.3）
2. 節拍音軌播放與視覺化拍點指示（跳動的節拍條）
3. 節奏題庫 10 題（含 beatMap 標注）
4. 建房設定加入「含節奏題」開關
...
| 3 | 單機對戰形式 | 同裝置兩人輪流（hot-seat），vs AI 留待日後 | 01 §1 |
| 4 | 前端技術棧 | React + TypeScript + Vite | 02 §1 |
| 5 | **先後攻是否交換** | **每回合交換**（你在需求裡問的問題——建議交換，理由見出處） | 01 §2.1 |
| 6 | 同回合被打到 0 | 後攻仍可完成朗讀；同歸於盡該回合重打 | 01 §3.2 |
| 7 | 道具取得方式 | 開場隨機發 3 個、每回合最多用 1 個、同時秘密選擇 | 01 §4 |
| 8 | 初始 HP / 傷害公式 | HP 100、damage = 20×accuracy² + 時間/完美加成（數值集中 balance.ts 可調） | 01 §3.2 |
| 9 | 節奏模式時程 | 放最後（Phase 4），因技術風險最高且不影響核心玩法 | 01 §3.3 |
| 10 | 帳號 / 配對 / 排行 | 初版不做，房號邀請制 | 02 §6 |
